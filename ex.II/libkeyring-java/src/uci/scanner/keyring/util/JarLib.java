package uci.scanner.keyring.util;
...
public class JarLib {
	...
	static public boolean load(Class cl, String libname) {
		...
		try { // Shouldn't really need to load from system defaults anymore
			...
			System.out.println("JarLib.load: Successfully loaded library ["
					+ libname + "] from some default system folder");
			return true;
		} catch (Exception e) {
			System.out.println("JarLib.load\n\tException = " + e.getMessage());
			System.err.println("JarLib.load\n\tException = " + e.getMessage());
	...
	static private void loadX(Class cl, String name) throws IOException,
			UnsatisfiedLinkError {
		String libname = System.mapLibraryName(name);
		URL url = cl.getResource("jni/" + libname);
		if (url == null) {
			throw new UnsatisfiedLinkError(JarLib.class.getName()
					+ ".loadX: Could not find library [" + libname + "]");
		}
		try {
...
//				System.out.println("JarLib.load: Successfully loaded library ["
//						+ url + "] from jascan standard file location");
			} else if (scheme.equals("jar")) { // make copy in tmp folder on local file system
				File dir = new File(System.getProperty("java.io.tmpdir"), "jascan");
				dir.mkdirs();
				...
				File tmp = File.createTempFile("jascan", libname, dir); // System.out.println(tmp.getAbsolutePath());
...
//						+ url + "] from jar file location");
				...
				throw new UnsatisfiedLinkError(
						JarLib.class.getName()
								+ ".loadX:\n\tUnknown URI-Scheme [+scheme+]; Could not load library ["
								+ uri + "]");
			}
		} catch (URISyntaxException urise) {
			throw new UnsatisfiedLinkError(
					JarLib.class.getName()
							+ ".loadX:\n\tURI-Syntax Exception; Could not load library ["
							+ url + "]");
		}
	}
